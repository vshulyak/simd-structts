...
def assert_models_equal(m1, m2, h, exog_predict=None):
    ...
    assert np.allclose(
        m1.get_forecast(h, exog=exog_predict).predicted_mean,
        m2.get_forecast(h, exog=exog_predict).predicted_mean,
    )
    assert np.allclose(
        m1.get_forecast(h, exog=exog_predict).se_mean ** 2,
        m2.get_forecast(h, exog=exog_predict).se_mean ** 2,
    )
